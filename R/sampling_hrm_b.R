...
#			kk <- 1
			...
			ll_new <- base::sum( base::log( probs_gpcm( x = xi[,ii] , theta , b=b_new[ii,] ,
   			                a=a[ii] , K=maxK[ii] , x_ind = xi_ind[,ii] ,
							useRcpp ) + eps) )
			ll_old  <- base::sum( base::log( probs_gpcm( x = xi[,ii] , theta , b=b_old[ii,] , 
							a=a[ii] , K=maxK[ii] , x_ind = xi_ind[,ii] ,
						    useRcpp ) + eps) )
			ratio <- p_new * base::exp( ll_new - ll_old ) / ( p_old  + eps )
    ...
    res <- base::list( b = b , MHprop = MHprop )
    base::return(res)
}
#####################################################################