        ...
        prior, MHprop, I, xi, useRcpp, est_settings  )
{
    ...
    if ( est.psi !='n' ){
        for (ii in 1L:I){
            ...
            if ( ( est.psi=='r' ) & ( ii==1 ) ){
                psi_new <- psi_old <- psi
                psi_new[1L:I,] <- matrix( rtrnorm( N=R, mean=psi[1,],
                                            sd=MHprop$SD$psi[1,], lower=rep(0,R),
                                            upper=rep(Inf,R) ),
                                    nrow=I, ncol=R, byrow=TRUE )
            }
            if ( ( est.psi=='e' ) & ( ii==1 ) ){
                psi_new <- psi_old <- psi
                psi_new[1L:I,1L:R] <- rtrnorm( N=1, mean=psi[1,1], sd=MHprop$SD$psi[1,1],
                                    lower=rep(0,1), upper=rep(Inf,1) )
            }
            if ( ( est.psi=='i' )  ){
                psi_new <- psi_old <- psi
                psi_new[ii,] <-  rep( rtrnorm( N=1, mean=psi[ii,1],
                                        sd=MHprop$SD$psi[ii,1],    lower=0, upper=Inf ),
                                        R )
            }
            if ( ( est.psi=='a' ) | ( ii==1 ) ){
                p_new <- stats::dnorm( psi_new[ii,], mean=prior$psi$M[ii,],
                                            sd=prior$psi$SD[ii,] )
                p_old <- stats::dnorm( psi_old[ii,], mean=prior$psi$M[ii,],
                                            sd=prior$psi$SD[ii,] )
            } else {
                p_new <- p_old <- rep(1,R)
            }
            ...
            ll_new <- probs_hrm( x=dat[,ii], xi=xi[ pid, ii ], phi=phi[ ii, rater ],
                                    psi=psi_new[ii,rater ], K=maxK[ii],
            ...
            ll_old <- probs_hrm( x=dat[,ii], xi=xi[ pid, ii ], phi=phi[ ii, rater ],
                                psi=psi_old[ii,rater ], K=maxK[ii],
                                x_ind=dat_ind[,ii], useRcpp=useRcpp)
            ...
            if ( est.psi=='i'){
                ...
                if ( ratio > stats::runif(1) ){
                    MHprop$accept$psi[ii,1L:R] <- MHprop$accept$psi[ii,1L:R] + 1
                    psi[ii,1L:R] <- psi_new[ii,1L:R]
                }
            }
        }  # end ii
        ...
        if ( est.psi=='r'){
            for (rr in 1L:R){
                ...
                if ( ratio[rr] > stats::runif(1) ){
                    MHprop$accept$psi[1L:I,rr] <- MHprop$accept$psi[1L:I,rr] + 1
                    psi[1L:I,rr] <- psi_new[1L:I,rr]
                }
            }  # end rr
        }
        if ( est.psi=='e'){
            ...
            if( is.na(ratio) ){ ratio <- 0 }
            if ( ratio > stats::runif(1) ){
                MHprop$accept$psi[1L:I,1L:R] <- MHprop$accept$psi[1L:I,1L:R] + 1
                psi[1L:I,1L:R] <- psi_new[1L:I,1L:R]
            }
        }
    } # end est.psi !='n'
    #--- output
