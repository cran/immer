...
sampling_hrm_a <- function( xi , xi_ind  , b , a , maxK , prior , MHprop , I , 
	...
	for (ii in 1:I ){
		a_new <- a_old <- a
		a_new[ii] <- stats::rlnorm( 1 , meanlog= base::log(a_old[ii]) , sdlog = MHprop$SD$a[ii] )			
		...
		ll_new <- base::sum( base::log( probs_gpcm( x = xi[,ii] , theta , b=b[ii,] , 
						a=a_new[ii] , K=maxK[ii] , x_ind = xi_ind[,ii] )  + eps) )
		ll_old  <- base::sum( base::log( probs_gpcm( x = xi[,ii] , theta , b=b[ii,] , 
    ...
    res <- base::list( a = a , MHprop = MHprop )
    base::return(res)
}
#####################################################################