...
sampling_hrm_phi <- function( dat , dat_ind , maxK , R , rater , pid , phi , psi ,
		...
		if ( est.phi != "n" ){
			...
			# ii <- 1				
				if ( est.phi=="a"){
					phi_new <- phi_old <- phi
					phi_new[ii,] <- stats::rnorm( R , mean= phi[ii,] , 
										sd = MHprop$SD$phi[ii,] )
				}
				if ( ( est.phi=="r" ) & ( ii == 1 ) ){
					phi_new <- phi_old <- phi
					phi_new[1:I,] <- base::matrix( stats::rnorm( R , mean= phi[1,] , 
										sd = MHprop$SD$phi[1,] ) ,
										nrow=I , ncol=R , byrow=TRUE )
				}																								
				if ( ( est.phi=="a" ) | ( ii == 1 ) ){
					p_new <- stats::dnorm( phi_new[ii,] , mean = prior$phi$M[ii,] , 
										sd = prior$phi$SD[ii,] ) 
					p_old <- stats::dnorm( phi_old[ii,] , mean = prior$phi$M[ii,] , 
										sd = prior$phi$SD[ii,] ) 
				} else {
		...
		res <- base::list( phi = phi , MHprop = MHprop )
        base::return(res)				
}