...
sampling_hrm_xi <- function( dat , theta , b , a , phi , psi , K  , pid , rater ,  ND ,
	...
	xi <- base::matrix( NA , nrow=N , ncol=I)
	eps <- 1E-20
	for (ii in 1:I){
		# ii <- 1
		x <- dat[,ii]
		...
		#	xi[,ii] <- base::.Call("immer_sampling_xi" ,  x_= x , theta , b[ii,] , a[ii] , 
...
sampling_hrm_xi_item <- function( x , theta , b , a , phi , psi , K  , pid , rater ,  ND=NULL ,
	...
	probs <- base::matrix( NA , nrow=N , ncol=K+1)	
	# calculate probability GPCM
	for (kk in 0:K){
		# kk <- 0	
		p1 <- probs_gpcm( x= base::rep(kk,N) , theta , b = b , a=a , K = K , x_ind = xi_ind , 
					useRcpp = useRcpp )				
		p2 <- probs_hrm( x = x , xi = base::rep( kk , ND ) , phi = phi[ rater ] , psi = psi[rater ] , K = K ,
					x_ind = x_ind , useRcpp)					
		p2 <- base::log( p2 + eps )				
		p3 <- base::rowsum( p2 , pid )		# sum over logarithms		
		probs[ , kk+1] <- p1 * base::exp( p3[,1] )			
	}					
	...
	if ( ! useRcpp ){
		...
		xi <- base::.Call("sample_prob_index" , as.matrix(probs) , rn , PACKAGE="immer")
	}												
	base::return(xi)
}
################################################################					